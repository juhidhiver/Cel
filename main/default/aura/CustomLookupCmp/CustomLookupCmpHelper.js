({
    checkIfInitialized: function(component, event, helper) {
        ...
        if (--initCallsRunning < 0) initCallsRunning = 0;
    ...
    addNewRecord: function(component, event, helper) {
        ...
        if (overrideNewEvent) {
			...
			*/
			addRecordEvent = $A.createComponent(
				"c:ChoosingRecordTypeCmp", 
				{
				...
				function(newButton, status) {
					if (status === "SUCCESS") {
                		component.find('overlayLib').showCustomModal({
							header: "New " + component.get('v.label'),
							body: newButton,
	...
	getObjLabel: function(component, event) {
        var action = component.get('c.getObjLabel');
        action.setParams({
        ...
        action.setCallback(this, function(response) {
            if (response.getState() === 'SUCCESS') {
				...
				//console.log('objlabel detected:='+response.getReturnValue() + 'test');
            }
