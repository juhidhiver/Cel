...
import getFFEndorsementByQuoteId from '@salesforce/apex/ManuscriptEndorsementControllerCel.getManuscriptEndorsementByQuoteId';
import getCoverageEffDate from '@salesforce/apex/ManuscriptEndorsementControllerCel.getCoverageEffDate';
import generateFormNumber from '@salesforce/apex/ManuscriptEndorsementControllerCel.generateFormNumber';
...
import MPL_Form_Number from '@salesforce/label/c.MPL_Form_Number';
import Cyber_Form_Number from '@salesforce/label/c.Cyber_Form_Number';
import PCC_Form_Number from '@salesforce/label/c.PCC_Form_Number';
...
export default class FreeFormEndorsementTabLwc extends LightningElement {
    ...
    @track columnsPI = [
        ...
        {
            label: 'Form number',
            fieldName: 'linkFFEndorsementName',
            type: 'url',
    ...
    wiregetProductName({ error, data }) {
        if (data) {
            ...
            /***** New if condition added based on MTA-107 ticket */
            if (this.productName == 'Private Company Combo') {
                console.log('this.productName'+this.productName)
                this.isApplicableCoverage = true;
                console.log(' this.isApplicableCoverage'+ this.isApplicableCoverage)
            }
            /***** New  condition ended based on MTA-107 ticket */
    ...
    setFormNumber() {
        if (this.productName == 'Cyber Standalone') {
            this.formNumber = Cyber_Form_Number;
        } else if (this.productName == 'MPL Standalone') {
            this.formNumber = MPL_Form_Number;
        } else if (this.productName == 'Private Company Combo') {
            this.formNumber = PCC_Form_Number;
        } else {
    ...
    handleOnLoad(event) {
        ...
        /*** new Function call based on ticket MTA-107 */
        this.getProductNameOnInit();
    }
    handleSubmit(event) {
    ...
    /****** New function added based on MTA-107 ticket */
    getProductNameOnInit(){

        getProductName({ quoteId: this.quoteId }).then(data => {
            this.productName = data;
            /***** New if condition added based on MTA-107 ticket */
            if (this.productName == 'Private Company Combo') {
                this.isApplicableCoverage = true;
